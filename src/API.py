...
class Application(db.Model):
    ìd = db.Column(db.Integer, primary_key=True)
    appname = db.Column(db.String(100))
    username = db.Column(db.String(100))
    last_connection = db.Column(db.TIMESTAMP(timezone=True))
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'))

    def __init__(self, appname, username, last_connection):
...
def populate_tables():
    ...
    for n in range(100):
        ...
        email = fake.email()
        job = fake.job()
        user = User(firstame, lastname, age, email, job)
